...
import { CostReportQuery, ProductTypeChoices } from '@/api/graphql';
import { useInfiniteProducts } from '@/api/hooks';
import { useCostReport } from '@/api/hooks/reports';
...
const ReportSuppliersOrders = () => {
    ...
    const reportQuery = useCostReport({
    ...
    if (reportQuery.error || (!reportQuery.data?.costReport && !reportQuery.isFetching)) {
        return <p>Error</p>;
    }
        ...
        >
            <div className="pr-container flex-1 bg-muted py-8 pl-8">
                ...
                {reportQuery.isFetching || !reportQuery.data ? (
                    ...
                    <Charts
                        report={reportQuery.data.costReport}
                        frequency={watchedFrequency}
